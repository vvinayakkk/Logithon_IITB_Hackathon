...
const CountryTab = () => {
  ...
  const fetchCountries = async () => {
    setError('');
    try {
      const response = await fetch('http://192.168.112.55:5000/api/countries');
  ...
  const fetchProhibitedItems = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`http://192.168.112.55:5000/api/country/${selectedCountry}`);
      const data = await response.json();
  ...
  const downloadPdf = async () => {
    ...
    try {
      ...
      const result = await FileSystem.downloadAsync(
        `http://192.168.112.55:5000/generate-pdf?data=${exportData}`,
  ...
  const sendPdfViaWhatsApp = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.112.55:5000/send_message', {
        method: 'POST',
        headers: {
  ...
  const sendPdfViaSms = async () => {
    ...
    try {
      ...
      const result = await FileSystem.downloadAsync(
        `http://192.168.112.55:5000/generate-country-pdf?data=${exportData}`,
        FileSystem.documentDirectory + filename
      );
